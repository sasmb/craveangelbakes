...
const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer
      ref={footerRef}
      className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 text-white relative flex flex-col w-full h-full justify-between lg:h-screen select-none"
    >
      {/* Seamless blend from CTA section */}
      <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-blue-600/80 via-blue-700/60 to-transparent"></div>
      
      <div className="relative z-10 container mx-auto flex flex-col md:flex-row justify-between w-full gap-4 pb-24 pt-8 px-4">
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-pink-400 transition-colors duration-200">
                  {link.label}
                </a>
              </li>
            ))}
          ...
          <p className="text-xs mt-2 opacity-70">powered by reech</p>
        </div>
        <div className="space-y-4">
          ...
          <div className="text-right mt-4">
            <button 
              ...
              className="text-sm hover:text-pink-400 hover:underline inline-flex items-center gap-1 transition-colors duration-200"
            >
              Back to top
            ...
            <div
              ...
              style={{
                ...
                background: index % 4 === 0 
                  ? "linear-gradient(90deg, rgb(236, 72, 153), rgb(219, 39, 119))" 
                  : index % 4 === 1 
                    ? "linear-gradient(90deg, rgb(59, 130, 246), rgb(37, 99, 235))"
                    : index % 4 === 2
                      ? "linear-gradient(90deg, rgb(167, 139, 250), rgb(139, 92, 246))"
